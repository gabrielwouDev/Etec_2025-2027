...
CREATE TABLE LimiteDeCredito (
    ...
    statusLimite VARCHAR(100) NOT NULL
);
...
-- INSERT
INSERT INTO LimiteDeCredito (id_limite, statusLimite) VALUES 
(1, 'BOM'), 
(2, 'REGULAR'), 
(3, 'RUIM');
...
CREATE VIEW vwClienteLimite AS
	SELECT nome `Nome`, statusLimite `Crédito` FROM LimiteDeCredito
		INNER JOIN Cliente ON LimiteDeCredito.id_limite = Cliente.id_limite 
